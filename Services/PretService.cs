...
public async Task<Pret> CreateAsync(CreatePretDTO dto, string role)
{
    ...
    if (equipement.etat == "Réformé")
        throw new Exception("L'équipement est réformé et ne peut pas être prêté.");

    if (equipement.etat == "Prêt")
        throw new Exception("L'équipement est déjà en prêt.");
